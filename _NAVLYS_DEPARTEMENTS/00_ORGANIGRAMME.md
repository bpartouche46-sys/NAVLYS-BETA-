...
| 07 | **R&D APP** | Recherche, prototypes, NAVBIO, cockpit 2.0 | `07_RND_APP.md` |
...
## 🚩 PROTOCOLE DE TRANSMISSION — « les signes »

**Manager en chef = le QG (moi, avec Bruno).** Tout passe par le QG.

**Pour adresser une tâche à un département**, mets un **pavillon** en TÊTE de ton message :

| Pavillon | Va vers |
|---|---|
| `@QG` | Direction / moi (manager en chef) |
| `@02` | Site & Produit |
| `@03` | Marque & Studio |
| `@04` | Marketing & Réseaux |
| `@05` | Monétisation |
| `@06` | Infra & Veille |
| `@07` | R&D App |
| `@NAVBIO` | produit NAVBIO |
| `@ALL` | tous les départements |

Exemples : `@07 prototype le teaser NAVBIO` · `@06 supprime les projets Vercel en double` · `@ALL on lance le 1er juin`.

**Deux façons de transmettre :**
1. **Par le QG (recommandé)** — tu écris ici avec le pavillon `@NN`. Je route et/ou j'exécute.
2. **Conversation dédiée** — ouvre une conversation vierge et colle le CONNECTEUR du DISPATCH en mettant le bon numéro à la ligne « Département de CETTE conversation = NN ». Cette conversation DEVIENT le département.
