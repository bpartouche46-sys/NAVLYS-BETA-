// NAVLYS — Webhook WhatsApp servi par Supabase. DOUBLE MODE d'envoi :
//   1) Meta Cloud API (GRATUIT) si META_ACCESS_TOKEN + META_PHONE_NUMBER_ID
//   2) sinon 360dialog si D360_API_KEY
// Reçoit le format Meta (entry/changes/value/messages), identique chez les deux.
// Mêmes secrets que le cockpit (SUPABASE_SERVICE_ROLE_KEY traverse RLS).
//
// Secrets Supabase (Edge Functions → Secrets) :
//   SUPABASE_URL, SUPABASE_SERVICE_ROLE_KEY, ANTHROPIC_API_KEY  (déjà là)
//   WHATSAPP_VERIFY_TOKEN = mot au choix (vérif webhook Meta)
//   BRUNO_WHATSAPP        = ton numéro en chiffres (mode pilote)
//   --- Mode GRATUIT Meta : ---
//   META_ACCESS_TOKEN     = token d'accès WhatsApp Cloud API
//   META_PHONE_NUMBER_ID  = ID du numéro (WhatsApp Manager)
//   --- ou repli 360dialog : ---
//   D360_API_KEY          = clé canal 360dialog
...
const hasChannel = () => (!!META_TOKEN && !!META_PHONE_ID) || !!D360;
const SYSTEM_SAV = [
...
async function sendWA(to, body){
  ...
  if (META_TOKEN && META_PHONE_ID) {
    r = await fetch("https://graph.facebook.com/v21.0/"+META_PHONE_ID+"/messages",{method:"POST",headers:{"Authorization":"Bearer "+META_TOKEN,"Content-Type":"application/json"},body:payload});
  } else if (D360) {
    r = await fetch("https://waba-v2.360dialog.io/messages",{method:"POST",headers:{"D360-API-KEY":D360,"Content-Type":"application/json"},body:payload});
  } else { return; }
  const t = await r.text().catch(()=>"");
  console.log("WA_SEND mode=", (META_TOKEN&&META_PHONE_ID)?"meta":"360", "status=", r.status, "resp=", t.slice(0,250));
}
function h(){ return { apikey:K, Authorization:"Bearer "+K, "Content-Type":"application/json" }; }
async function sbGet(table,select,order,limit){
  const url=(ord)=>`${U}/rest/v1/${table}?select=${encodeURIComponent(select)}${ord?`&order=${encodeURIComponent(ord)}`:""}${limit?`&limit=${limit}`:""}`;
  let r=await fetch(url(order),{headers:h()}); if(!r.ok&&order) r=await fetch(url(null),{headers:h()});
  return r.ok? await r.json().catch(()=>[]):[];
}
async function sbFilter(table,select,filter,limit){
...
function routeDept(text, valid){
  ...
  for(const dept of Object.keys(KEYWORDS)) if(valid.includes(dept)&&KEYWORDS[dept].some((w)=>low.includes(w))) return dept;
...
async function pilote(text){
  ...
  const ags=await sbGet("agents","id","id.asc",50); const valid=ags.map((a)=>a.id);
...
Deno.serve(async (req) => {
  ...
  if (req.method === "GET") {
    if (u.searchParams.get("diag")) {
      return new Response(JSON.stringify({ channel: (META_TOKEN&&META_PHONE_ID)?"meta":(D360?"360dialog":"AUCUN"), metaToken:META_TOKEN.length, metaPhoneId:META_PHONE_ID.length, d360:D360.length, bruno:BRUNO, anth:ANTH.length, url:U.length, key:K.length }),{status:200,headers:{"Content-Type":"application/json"}});
    }
  ...
  try{
    ...
    if(msg && msg.type==="text" && hasChannel()){
