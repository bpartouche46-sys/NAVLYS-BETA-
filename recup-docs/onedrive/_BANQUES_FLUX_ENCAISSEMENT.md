...
- **Stripe Israel** : disponibilité contestée dans la doc (cf. `NOVA_BRIEFING_TRANSFERT.md`). **À TRANCHER** : entité Stripe = [entité — hors dépôt] FR (déjà existant) ou IL (nouveau). **Recommandation** : utiliser [entité — hors dépôt] FR (SIREN 482 511 292) car compte EI déjà ouvert + Stripe France disponible immédiatement → contournement du blocage IL.
...
```
STRIPE ([entité — hors dépôt] FR) → WISE BUSINESS EUR (pivot) → splits vers :
